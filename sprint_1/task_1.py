# For the given integer n, consider an increasing sequence consisting 
# of all positive integers that are either powers of n, or sums of distinct powers of n.
...
def kthTerm(n, k):
    ...
    while len(result) < k:
        ...
        for value in result:
            current_sum = value + current_power
            if current_sum not in temp_result:
                temp_result.append(current_sum)
        temp_result.sort()
        result = temp_result
        power += 1
    return result[k - 1]
